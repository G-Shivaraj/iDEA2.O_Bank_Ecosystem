...
export default function AlertPanel() {
  ...
  const getSeverityBadge = (severity) => {
    switch (severity) {
      case 'CRITICAL':
        return 'shadow-[0_0_12px_rgba(239,68,68,0.45)] bg-rose-100 dark:bg-rose-500/20 border-rose-500 text-rose-300';
      case 'HIGH':
      ...
      default:
        return 'bg-slate-50 dark:bg-slate-500/10 text-slate-500 dark:text-slate-400 border border-slate-500/30';
    }
  };
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'UNRESOLVED':
        return 'bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20';
      case 'ACKNOWLEDGED':
        return 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20';
      case 'RESOLVED':
        return 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20';
      default:
    ...
    <div className="flex flex-col gap-8 w-full max-w-[92rem] mx-auto pb-12 h-full">
      ...
      <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800/80 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5">Real-time incident response queue and risk mitigations (polling 10s)</p>
        </div>
      ...
      {/* Filter Toolbar */}
      <div className="bg-white dark:bg-[#111827]/80 backdrop-blur border border-slate-200 dark:border-slate-800/80 p-5 rounded-xl shadow-lg flex flex-wrap gap-5 items-center justify-between">
        <div className="flex flex-wrap gap-5 items-center">
          <div className="flex items-center gap-3 text-sm font-semibold text-slate-500 dark:text-slate-400">
            <Filter size={16} className="text-blue-600 dark:text-blue-400" />
            <span>Severity:</span>
            <div className="flex rounded-lg bg-slate-50 dark:bg-slate-950 p-1 border border-slate-200 dark:border-slate-800/60">
      ...
      {loading ? (
        <div className="h-[45vh] flex flex-col items-center justify-center gap-3">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400 tracking-widest font-mono uppercase animate-pulse">Querying real-time incident storage...</span>
        </div>
      ) : (
            ...
            {filteredAlerts.map((alert) => {
              ...
              return (
                <motion.div
                  ...
                  key={alert.id}
                  className="bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur border border-slate-200 dark:border-slate-700 rounded-xl p-8 shadow-2xl flex flex-col justify-between hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-200 relative group overflow-hidden min-h-[250px] h-auto shrink-0 gap-5"
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 w-full border-b border-slate-200 dark:border-slate-800/40 pb-3">
                    ...
                    <span className="text-[13px] text-slate-500 dark:text-slate-400 font-mono font-semibold">
                  ...
                  <div className="w-full">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3.5 text-sm font-mono text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/50 p-5 rounded-lg border border-slate-200 dark:border-slate-900/60">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-black text-[12px] px-2.5 py-0.5 rounded ${
                          ...
                          risk > 60 ? 'bg-amber-50 dark:bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/25' :
                          'bg-blue-50 dark:bg-blue-500/15 text-blue-600 dark:text-blue-400 border border-blue-500/25'
                        }`}>
                          {risk}
                        </span>
                      </div>
                  ...
                  <div className="pt-4 border-t border-slate-200 dark:border-slate-800/60 flex flex-wrap justify-end gap-3 w-full mt-2">
                    ...
                    {alert.status === 'UNRESOLVED' && (
                      <button
                        ...
                        className="px-4 py-2 bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-100 dark:hover:bg-amber-500/20 border border-amber-500/20 hover:border-amber-500/40 text-amber-600 dark:text-amber-400 rounded-lg transition-all text-sm font-bold uppercase tracking-wider flex items-center gap-1.5"
                      >
                        <Eye size={14} />
                        Acknowledge
                      </button>
                    ...
                    {alert.status !== 'RESOLVED' && (
                      <button
                        ...
                        className="px-4 py-2 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 border border-emerald-500/20 hover:border-emerald-500/40 text-emerald-600 dark:text-emerald-400 rounded-lg transition-all text-sm font-bold uppercase tracking-wider flex items-center gap-1.5"
                      >
                        <CheckCircle size={14} />
                        Resolve Incident
                      </button>
