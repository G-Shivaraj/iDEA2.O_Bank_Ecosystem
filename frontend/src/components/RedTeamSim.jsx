...
const SCENARIO_META = {
  ...
  credential_stuffing: {
    ...
    icon: Compass,
    color: "from-blue-500/20 to-indigo-500/20 border-blue-500/30 text-blue-400 hover:border-blue-500/50",
  ...
  ransomware_spread: {
    ...
    icon: Flame,
    color: "from-amber-500/20 to-orange-600/20 border-amber-500/30 text-amber-400 hover:border-amber-500/50",
    ...
    <div className="flex flex-col gap-8 w-full max-w-[92rem] mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div className="xl:col-span-6 flex flex-col gap-6">
          <div className="bg-[#0f172a]/95 backdrop-blur border border-slate-750 rounded-xl p-7 shadow-2xl">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 border-b border-slate-800 pb-3 mb-6 flex items-center gap-2.5">
            ...
            {loadingScenarios ? (
              <div className="py-32 flex flex-col items-center justify-center gap-3">
                ...
                <span className="text-[10px] text-slate-500 tracking-wider font-mono">LOADING SIMULATOR DIRECTORY...</span>
              </div>
            ) : (
              <div className="flex flex-col gap-5">
                {scenarios.map((id) => {
                  const meta = SCENARIO_META[id] || {
                    ...
                    icon: ShieldAlert,
                    color: "border-slate-800 bg-slate-900/35 hover:border-slate-700 text-slate-300",
                  ...
                  return (
                    <motion.div
                      ...
                      key={id}
                      className={`p-6 rounded-xl border bg-slate-950/20 transition-all duration-300 flex flex-col justify-between gap-5 ${meta.color}`}
                    >
                      <div className="flex justify-between items-start gap-4">
                        ...
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded border uppercase tracking-wider ${
                          id === 'swift_fraud' ? 'bg-red-500/10 text-red-400 border-red-500/25 shadow-[0_0_8px_rgba(239,68,68,0.15)]' :
                      ...
                      <div className="flex justify-between items-center pt-4 border-t border-slate-900 text-[11px] font-mono text-slate-500">
                        <div className="flex gap-4">
                          <span>Start: <span className="text-cyan-400 font-bold">{id === 'credential_stuffing' || id === 'lateral_movement' ? 'Asset_1 (Gateway)' : id === 'insider_threat' ? 'Asset_2 (Auth Router)' : 'Asset_3 (Host)'}</span></span>
        ...
        <div className="xl:col-span-6 flex flex-col gap-8">
          ...
          <div className="bg-slate-950 border border-slate-750 rounded-xl overflow-hidden shadow-2xl flex flex-col h-[520px] relative before:absolute before:inset-0 before:bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(255,0,0,0.06),rgba(0,255,0,0.02),rgba(0,0,255,0.06))] before:bg-[size:100%_4px,3px_100%] before:pointer-events-none before:z-10">
            <div className="px-5 py-3.5 bg-slate-900/60 border-b border-slate-800 flex justify-between items-center font-mono relative z-20">
              ...
              <span className="text-[10px] text-slate-500 font-mono font-bold">CYBERDEFENSE_SIMULATOR_v1.0</span>
            </div>

            <div className="flex-1 p-5 overflow-y-auto font-mono text-[12px] leading-relaxed text-slate-300 flex flex-col gap-2.5 custom-scrollbar relative z-20">
              {!simRunning ? (
                <div className="m-auto text-center text-slate-600 flex flex-col items-center gap-2">
                ...
                <>
                  ...
                  {simulationSteps.slice(0, currentStepIndex + 1).map((step, idx) => {
                    ...
                    let statusColor = "text-amber-400 border border-amber-500/20 bg-amber-500/5";
                    ...
                    return (
                      <motion.div
                        ...
                        key={step.id || idx}
                        className="flex flex-col gap-1.5 p-3.5 bg-slate-900/75 border border-slate-800 rounded-lg shadow-md mb-2"
                      >
                        <div className="flex justify-between items-center gap-4 text-[11px]">
                          <span className="text-slate-500">[{step.timestamp?.replace('T', ' ')}]</span>
                          <span className="text-cyan-400 font-bold uppercase">{step.asset_id}</span>
                        </div>
                        <p className="text-slate-200 text-xs">{step.message}</p>
                  ...
                  {currentStepIndex < simulationSteps.length - 1 && (
                    <div className="flex items-center gap-1.5 text-cyan-400 font-bold animate-pulse mt-2 text-xs">
          ...
          <AnimatePresence>
            {simVerdict && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -15 }}
                className="bg-[#0f172a]/95 backdrop-blur border border-slate-750 rounded-xl p-7 shadow-2xl flex flex-col gap-6"
              >
                <div className="flex justify-between items-center border-b border-slate-800 pb-4">
                  ...
                  <span className={`text-[11px] font-black px-3 py-1.5 rounded-lg border uppercase tracking-wider transition-all duration-300 ${
                    simVerdict.verdict === 'CRITICAL GAPS FOUND' ? 'shadow-[0_0_15px_rgba(239,68,68,0.5)] bg-red-500/20 border-red-500 text-red-300' :
                    simVerdict.verdict === 'MODERATE GAPS' ? 'shadow-[0_0_15px_rgba(245,158,11,0.5)] bg-amber-500/20 border-amber-500 text-amber-300' :
                ...
                <p className="text-[13px] text-slate-300 leading-relaxed font-mono bg-slate-950/60 p-4.5 rounded-xl border border-slate-900">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-slate-950/40 p-4.5 rounded-xl border border-slate-900 flex flex-col gap-1.5">
                    <span className="text-[10px] font-mono text-slate-500 uppercase">Success Penetration Rate</span>
                  ...
                  <div className="bg-slate-950/40 p-4.5 rounded-xl border border-slate-900 flex flex-col gap-1.5">
                    <span className="text-[10px] font-mono text-slate-500 uppercase">Assets Breached</span>
                    <span className="text-3xl font-bold font-mono text-white flex items-center gap-2">
                ...
                <div className="flex flex-col gap-3 pt-3 border-t border-slate-900">
                  <span className="text-[10px] font-mono text-slate-500 uppercase">Discovered Vulnerability Links</span>
                  <div className="flex flex-wrap gap-3">
                    {simVerdict.discoveredCVEs.map((cve, idx) => (
                      <div 
                        key={idx}
                        className="px-3.5 py-2 bg-slate-950/80 border border-slate-800 hover:border-cyan-500/40 rounded-xl flex items-center justify-between gap-4 text-xs font-mono text-slate-300 w-full sm:w-auto"
                      >
                        <span>{cve}</span>
                        <button
                          onClick={() => handleNavigatePlaybook(cve)}
                          className="text-[10px] font-bold text-cyan-400 hover:text-cyan-300 uppercase tracking-wide flex items-center gap-0.5 border-l border-slate-800 pl-3.5"
                        >
