...
const SCENARIO_META = {
  lateral_movement: {
    ...
    icon: Layers,
    color: "from-orange-500/20 to-red-500/20 border-orange-500/30 text-orange-600 dark:text-orange-400 hover:border-orange-500/50",
    badgeColor: "bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20"
  },
  credential_stuffing: {
  ...
  swift_fraud: {
    ...
    icon: Cpu,
    color: "from-red-500/20 to-rose-600/20 border-red-500/30 text-rose-600 dark:text-rose-400 hover:border-red-500/50 animate-pulse",
    badgeColor: "bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20"
  },
  ransomware_spread: {
  ...
  insider_threat: {
    ...
    icon: ShieldAlert,
    color: "from-purple-500/20 to-indigo-600/20 border-purple-500/30 text-purple-600 dark:text-purple-400 hover:border-purple-500/50",
    badgeColor: "bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20"
  }
};
    ...
    <div className="flex flex-col gap-8 w-full max-w-[92rem] mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div className="xl:col-span-6 flex flex-col gap-6">
          <div className="bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur border border-slate-200 dark:border-slate-700 rounded-xl p-7 shadow-2xl">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 pb-3 mb-6 flex items-center gap-2.5">
              ...
              <div className="flex flex-col gap-5">
                {scenarios.map((id) => {
                  const meta = SCENARIO_META[id] || {
                    ...
                    icon: ShieldAlert,
                    color: "border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/35 hover:border-slate-300 dark:hover:border-slate-700 text-slate-700 dark:text-slate-300",
                    ...
                    >
                      <div className="flex justify-between items-start gap-4">
                        <div className="flex items-center gap-4">
                          <div className={`p-3.5 rounded-lg bg-slate-100 dark:bg-slate-950/80 border ${meta.badgeColor.replace('bg-', 'border-')}`}>
                            <Icon size={24} />
                          </div>
                          <div>
                        ...
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded border uppercase tracking-wider ${id === 'swift_fraud' ? 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/25 shadow-[0_0_8px_rgba(239,68,68,0.15)]' :
                            id === 'insider_threat' || id === 'lateral_movement' ? 'bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/25' :
                      ...
                      <div className="flex justify-between items-center pt-4 border-t border-slate-200 dark:border-slate-900 text-[11px] font-mono text-slate-500 dark:text-slate-500">
                        <div className="flex gap-4">
                          ...
                          <span>Target: <span className="text-rose-600 dark:text-rose-400 font-bold">{id === 'swift_fraud' ? 'Asset_5 (SWIFT)' : id === 'credential_stuffing' ? 'Asset_2 (Auth Router)' : 'Asset_4 (DB)'}</span></span>
                        </div>

                        <button
                          disabled={simRunning}
                          onClick={() => handleSimulate(id)}
                          className="px-4.5 py-2.5 bg-rose-50 dark:bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 hover:border-rose-500/40 disabled:border-slate-200 dark:disabled:border-slate-800 disabled:bg-slate-100 dark:disabled:bg-slate-900/10 disabled:text-slate-400 dark:disabled:text-slate-600 text-rose-600 dark:text-rose-400 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all duration-200 uppercase tracking-wide"
                        >
        ...
        <div className="xl:col-span-6 flex flex-col gap-8">
          {/* Terminal Console */}
          <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-2xl flex flex-col h-[520px] relative before:absolute before:inset-0 before:bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(255,0,0,0.06),rgba(0,255,0,0.02),rgba(0,0,255,0.06))] before:bg-[size:100%_4px,3px_100%] before:pointer-events-none before:z-10 before:opacity-0 dark:before:opacity-100">
            ...
            <div className="flex-1 p-5 overflow-y-auto font-mono text-[12px] leading-relaxed text-slate-700 dark:text-slate-300 flex flex-col gap-2.5 custom-scrollbar relative z-20">
              {!simRunning ? (
                <div className="m-auto text-center text-slate-500 dark:text-slate-600 flex flex-col items-center gap-2">
                  <Terminal size={32} className="text-slate-300 dark:text-slate-800" />
                ...
                <>
                  ...
                  {simulationSteps.slice(0, currentStepIndex + 1).map((step, idx) => {
                    ...
                    if (isSuccess) statusColor = "text-rose-600 dark:text-rose-400 border border-rose-500/20 bg-rose-500/5";
                    if (isBlocked) statusColor = "text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 bg-emerald-500/5";
                    ...
                    return (
                      <motion.div
                        ...
                        key={step.id || idx}
                        className="flex flex-col gap-1.5 p-3.5 bg-slate-50 dark:bg-slate-900/75 border border-slate-200 dark:border-slate-800 rounded-lg shadow-md mb-2"
                      >
                        ...
                        <div className="flex gap-2 mt-1">
                          <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400">
          ...
          <AnimatePresence>
            {simVerdict && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -15 }}
                className="bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur border border-slate-200 dark:border-slate-700 rounded-xl p-7 shadow-2xl flex flex-col gap-6"
              >
                <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-4">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-2.5">
                    <Skull size={16} className="text-rose-500" />
                    Adversary Assessment Report
                  </h3>
                  <span className={`text-[11px] font-black px-3 py-1.5 rounded-lg border uppercase tracking-wider transition-all duration-300 ${simVerdict.verdict === 'CRITICAL GAPS FOUND' ? 'shadow-[0_0_15px_rgba(239,68,68,0.5)] bg-red-100 dark:bg-red-500/20 border-red-500 text-red-700 dark:text-red-300' :
                      simVerdict.verdict === 'MODERATE GAPS' ? 'shadow-[0_0_15px_rgba(245,158,11,0.5)] bg-amber-100 dark:bg-amber-500/20 border-amber-500 text-amber-700 dark:text-amber-300' :
                        'shadow-[0_0_15px_rgba(34,197,94,0.5)] bg-emerald-100 dark:bg-emerald-500/20 border-emerald-500 text-emerald-700 dark:text-emerald-300'
                    }`}>
                    {simVerdict.verdict}
                  </span>
                </div>

                <p className="text-[13px] text-slate-700 dark:text-slate-300 leading-relaxed font-mono bg-slate-50 dark:bg-slate-950/60 p-4.5 rounded-xl border border-slate-200 dark:border-slate-900">
                  {simVerdict.executiveSummary}
                </p>

                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-slate-50 dark:bg-slate-950/40 p-4.5 rounded-xl border border-slate-200 dark:border-slate-900 flex flex-col gap-1.5">
                    <span className="text-[10px] font-mono text-slate-500 dark:text-slate-500 uppercase">Success Penetration Rate</span>
                    <span className="text-3xl font-bold font-mono text-slate-900 dark:text-white flex items-center gap-2">
                ...
                <div className="flex flex-col gap-3 pt-3 border-t border-slate-200 dark:border-slate-900">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {simVerdict.discoveredCVEs.map((cve, idx) => (
                      <div
                        key={idx}
                        className="px-3.5 py-2 bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 hover:border-cyan-500/40 rounded-xl flex items-center justify-between gap-4 text-xs font-mono text-slate-700 dark:text-slate-300 w-full sm:w-auto"
                      >
                        <span>{cve}</span>
                        <button
